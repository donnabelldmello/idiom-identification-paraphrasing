...
class hmmdecode():
	...
	def main(self, args):
		test_file = list(io.open(args[0], encoding='utf-8'));
		self.loadParams();
	...
	def decodeLine(self, line):
		...
		for t in range(1,len(words)):
			...
			for iTag in range(0,len(self.tags)):
				...
				maxProb, iPTagId = max((probability[iPTagId][t-1] + math.log(self.transitionP.get(iPTag).get(tag)) + eVal, iPTagId) for iPTagId, iPTag in enumerate(self.tags))
				probability[iTag][t] = maxProb;
				backpointer[iTag][t] = iPTagId
			#print np.matrix(probability)

		# Termination step
		p, most_probable_tagId = max((probability[iPTagId][len(words) - 1], iPTagId) for iPTagId, iPTag in enumerate(self.tags))
